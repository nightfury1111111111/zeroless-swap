...
export function useFoundOnInactiveList(searchQuery: string, isBridge: boolean): Token[] | undefined {
  const { chainId } = useActiveWeb3React()
  ...
  return useMemo(() => {
    ...
    const tokens = filterTokens(Object.values(inactiveTokens), searchQuery, isBridge)
    return tokens
