...
const Container = styled.div<{ card: boolean }>`
  ...
  ${({ theme }) => theme.mediaQueries.sm} {
    width: ${(card) => (card.card ? '100%' : '50%')};
  }
`
export default function RemoveLiquidityWidget({ farm, card }) {
  ...
  const currencyA1 = liquidityPairA || farm.token[chainId]?.address[chainId]
  ...
  else currencyB1 = liquidityPairB || farm.quoteToken[chainId]?.address[chainId]
