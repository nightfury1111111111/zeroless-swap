...
/**
 * Returns all the pairs of tokens that are untracked by the user
 */
...
export function useUnTrackedTokenPairs(): [Token, Token][] {
  const { chainId } = useActiveWeb3React()
      ...
      chainId
        ? flatMap(Object.keys(tokens), (tokenAddress) => {
            ...
            return (
              ...
              Object.keys(tokens)
                .map((address) => {
                  if (token === tokens[address]) {
                    return null
                  }
                  return [token, tokens[address]]
                })
                .filter((p): p is [Token, Token] => p !== null)
            )
          })
