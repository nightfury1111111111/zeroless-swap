...
import { useBondRedeemHelper, useBondCalculator } from 'hooks/useContract';
import { getBalances, calculateUserBondDetails } from "./AccountSlice";
import { AppState } from 'state';
import { clearPendingTxn, fetchPendingTxns } from "./PendingSlice";
...
export const changeApproval = createAsyncThunk(
  ...
  async ({ address, bond, provider, networkID }: IApproveBondAsyncThunk, { dispatch }) => {
    ...
    const error = (message: string) => toastError('Error', message)
    const info = (message: string) => toastSuccess('Error', message)
    ...
    try {
      ...
      error((e as IJsonRPCError).message);
    } finally {
      if (approveTx) {
...
export const calcBondDetails = createAsyncThunk(
  ...
  async ({ bond, value, provider, networkID }: ICalcBondDetailsAsyncThunk, { dispatch }): Promise<IBondDetails> => {
    ...
    const bondCalcContract = useBondCalculator();
...
export const redeemAllBonds = createAsyncThunk(
  ...
  async ({ bonds, address, networkID, provider, autostake }: IRedeemAllBondsAsyncThunk, { dispatch }) => {
    ...
    const redeemHelperContract = useBondRedeemHelper();
    let redeemAllTx;
...
const baseInfo = (state: AppState) => state.bonding;
